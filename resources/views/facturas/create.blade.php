    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('facturas.store') }}" method="POST" class="p-6">
                    @csrf
                    <div class="mb-4">
                        ...
                        <select name="tipo_documento" id="tipo_documento" class="form-select rounded-md shadow-sm mt-1 block w-full" required autofocus>
                            ...
                            <option value="Factura de Venta POS"{{ old('tipo_documento') == 'Factura de Venta POS' ? ' selected' : '' }}>Factura de Venta POS</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="tipo_resolucion" id="tipo_resolucion" class="form-select rounded-md shadow-sm mt-1 block w-full" required>
                            ...
                            @foreach ($tipos_resolucion as $tipo_resolucion)
                                <option value="{{ $tipo_resolucion }}"{{ old('tipo_resolucion') == $tipo_resolucion ? ' selected' : '' }}>{{ $tipo_resolucion }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="numero_resolucion" class="block text-gray-700 font-bold mb-2">Número de resolución:</label>
                        <input type="text" name="numero_resolucion" id="numero_resolucion" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('numero_resolucion') }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="prefijo_facturacion" class="block text-gray-700 font-bold mb-2">Prefijo de facturación:</label>
                        <input type="text" name="prefijo_facturacion" id="prefijo_facturacion" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('prefijo_facturacion') }}">
                    </div>
                    <div class="mb-4">
                        <label for="fecha_autorizacion" class="block text-gray-700 font-bold mb-2">Fecha de autorización:</label>
                        <input type="date" name="fecha_autorizacion" id="fecha_autorizacion" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('fecha_autorizacion') }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="numero_factura_inicial" class="block text-gray-700 font-bold mb-2">Número de factura inicial:</label>
                        <input type="number" name="numero_factura_inicial" id="numero_factura_inicial" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('numero_factura_inicial') }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="numero_factura_final" class="block text-gray-700 font-bold mb-2">Número de factura final:</label>
                        <input type="number" name="numero_factura_final" id="numero_factura_final" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('numero_factura_final') }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="fecha_inicio" class="block text-gray-700 font-bold mb-2">Fecha de inicio:</label>
                        <input type="date" name="fecha_inicio" id="fecha_inicio" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('fecha_inicio') }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="fecha_vencimiento" class="block text-gray-700 font-bold mb-2">Fecha de vencimiento:</label>
                        <input type="date" name="fecha_vencimiento" id="fecha_vencimiento" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('fecha_vencimiento') }}" required>
                    </div>
